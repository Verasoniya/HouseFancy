...
import { FaChevronCircleLeft, FaChevronCircleRight, FaMapMarker, FaPhone, FaWhatsapp, FaEnvelope, FaUserAlt } from "react-icons/fa";
...
const DetailHouseList = () => {
  ...
  const fetchDetail = () => {
    apiRequest(`houses/2`, `GET`, {}).then((res) => {
      const data = res.data;
      const image = [];
    ...
    <Layout>
      <div className="flex justify-center w-full mt-10">
        <div className="flex flex-col items-center w-5/6 gap-8">
              ...
              {house.image_url.map((slide, index) => {
                return (
                  <div className={index === current ? "slide active" : "slide"} key={index}>
                    {index === current && <img src={slide.image_url} alt="slide-img" className="w-[800px] h-[200px] lg:h-[300px]" />}
                  </div>
                );
              })}
            </div>
          ...
          <p className="font-bold text-lg self-start">{"Elegant house with strategic location"}</p>
          <div className="flex flex-col lg:flex-row justify-between w-full">
            <div className="flex flex-col w-full lg:w-1/2">
              ...
              <TrTdDescription title={"Description:"} content={house.location} />
            </div>
            <div className="flex flex-col w-full lg:w-2/5 mt-8 lg:mt-0">
              <div className="flex justify-between  bg-white shadow shadow-slate-700 p-4">
                <div className="flex flex-col gap-3">
                  <div className="flex gap-2">
                    <FaUserAlt className="text-md self-center" /> <p className="font-thin text-md">Fullname</p>
                  </div>
                  <div className="flex gap-2">
                    <FaPhone className="text-md self-center" /> <p className="font-thin text-md">0831-2748-4826</p>
                  </div>
                  <div className="flex gap-2">
                    <FaEnvelope className="text-md self-center" /> <p className="font-thin text-md">name@mail.com</p>
                  </div>
                  <div className="flex gap-2">
                    <FaMapMarker className="text-md self-center" /> <p className="font-thin text-md">Location</p>
                  </div>
                </div>
                <img src={logoblue} alt="logo" width={50} height={50} className="rounded-full self-center mr-2" />
              </div>
              <div className="w-full mt-8 z-0">
                <MapContainer center={[long, lat]} zoom={13} scrollWheelZoom={true} style={{ height: "250px" }}>
              ...
              <div className="mt-5">
                <Input id={"input-bidder"} type={"text"} placeholder={"Add Bid Amount"} />
              </div>
              <div className="mt-3">
                <CustomButton id={"button-bidder"} label={"BID NOW"} color={"blue"} />
              </div>
              <div className="mt-5">
                <CustomButton id={"button-whatsapp"} icon={<FaWhatsapp className="text-2xl mr-2" />} label={"WHATSAPP"} color={"green"} />
              </div>
            </div>
          </div>
          <div className="border-t border-dashed border-blue-400 w-full mt-16" />
          <p className="font-semibold text-2xl self-start">Bidder</p>
          <div className="self-start w-2/3">{/* <BidderHouse /> */}</div>
        </div>
      </div>
    </Layout>
  );
