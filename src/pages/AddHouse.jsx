...
function AddHouse() {
  ...
  const handleSubmitImages = async (id_house) => {
    ...
    for (let i = 0; i < image.length; i++) {
      ...
      requests.push(
        apiRequest(`/houses/images/${id_house}`, "POST", formData, "multipart/form-data")
          .then(async (res) => {
      ...
      <Layout>
        <div className="flex flex-col items-center my-10">
          ...
          <form className="flex flex-col w-full gap-1 lg:w-2/5 mb-10 px-4 lg:px-0" onSubmit={(e) => handleSubmit(e)}>
            ...
            <Input type={"text"} id={"input-house-title"} placeholder={"House Title"} required onChange={(e) => setTitle(e.target.value)} />

            <Label label={"House Price"} />
            <Input type={"number"} id={"input-house-price"} placeholder={"House Price"} required onChange={(e) => setPrice(e.target.value)} />

            <Label label={"House Location"} />
            <Input type={"text"} id={"input-house-location"} placeholder={"House Location"} required onChange={(e) => setLocation(e.target.value)} />
            <div className="z-0">
            ...
            <Input type={"number"} id={"input-house-surface-area"} placeholder={"Surface Area"} required onChange={(e) => setSurfaceArea(e.target.value)} />

            <Label label={"Building Area"} />
            <Input type={"number"} id={"input-house-building-area"} placeholder={"Building Area"} required onChange={(e) => setBuildingArea(e.target.value)} />

            <Label label={"Number of Bedrooms"} />
            <Input type={"number"} id={"input-house-bedrooms"} placeholder={"Number of Bedrooms"} required onChange={(e) => setBedroom(e.target.value)} />

            <Label label={"Number of Bathrooms"} />
            <Input type={"number"} id={"input-house-bathrooms"} placeholder={"Number of Bathrooms"} required onChange={(e) => setBathroom(e.target.value)} />

            <Label label={"House Certificate (SHM/HGB)"} />
            <Input type={"text"} id={"input-house-certificate"} placeholder={"House Certificate (SHM/HGB)"} required onChange={(e) => setCertificate(e.target.value)} />
